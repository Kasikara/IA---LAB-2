...
var dfsSearch = DFS.Search(graph, startC, endC);   /// 1 ///
if (dfsSearch != null)
{
	Console.WriteLine($"Shortest path from \"{citiesIndex[startC]}\" to \"{citiesIndex[endC]}\":");
...
var path4 = BidirectionalSearch.BiDirSearch(graph, startC, endC);  /// 2 ///
if (path4 != null)
{
	Console.WriteLine($"Shortest path from \"{citiesIndex[startC]}\" to \"{citiesIndex[endC]}\":");
...
var heuristicDistances = new Dictionary<int, int>()    /// 3 ///
		{
...
List<int> path9 = GreedyBestFirstSearch.FindPath(graph.adjList, heuristicDistances, startC, endC); /// 4 ///
...
var graph2 = new List<List<AStar.Node2>>(); /// 5 ///
...
UniformCostSearch ucs = new UniformCostSearch();   /// 6 ///
